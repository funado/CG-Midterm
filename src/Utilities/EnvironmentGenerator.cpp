	...
	for (int i = 0; i < _objectsToSpawn.size(); i++)
	{
		std::vector<GameObject> temp;
		{
			...
			for (int j = 0; j < _numToSpawn[i]; j++)
			{
				...
				temp[j].get<Transform>().SetLocalRotation(Util::GetRandomNumberBetween(glm::vec3(85.0f, 0.0f, 0.0f), glm::vec3(95.0f, 0.0f, 360.0f)));
				temp[j].get<Transform>().SetLocalScale(Util::GetRandomNumberBetween(glm::vec3(1.0f, 1.0f, 1.0f), glm::vec3(2.0f, 2.5f, 2.0f)));
				//temp[j].get<Transform>().SetLocalRotation(glm::vec3(90.0f, 0.0f, 0.0f));
			}
		}
