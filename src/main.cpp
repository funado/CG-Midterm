	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (activeEffect == 3)
			{
				...
				if (ImGui::SliderFloat("threshold", &threshold, 0.01f, 0.5f))
				{
			...
			if (ImGui::Checkbox("Ambient Light Only", &amb)) {
				none = false;
				amb = true;
				spec = false;
				useBloom = false;
			...
			if (ImGui::Checkbox("Specular Light Only", &spec)) {
				none = false;
				amb = false;
				spec = true;
				useBloom = false;
			...
			if (ImGui::Checkbox("Ambient and Specular Light", &ambSpec)) {
				none = false;
				useBloom = false;
				ambSpecBloom = false;
				if (ambSpec)
			...
			if (ImGui::Checkbox("Texture Toggle", &noTex))
			{
				if (noTex)
				{
			...
			//	if (activeEffect == 4)
			//	{
			//		ImGui::Text("Active Effect: Blur Effect");
			//
			//		GaussianBlur* temp = (GaussianBlur*)effects[activeEffect];
			//		float intensity = temp->GetIntensity();
			//
			//		if (ImGui::SliderFloat("Intensity", &intensity, 0.0f, 1.0f))
			//		{
			...
			//	if (activeEffect == 5)
			//	{
			//		ImGui::Text("Active Effect: Bloom Final Effect");
			//
			//		//GaussianBlur* temp = (GaussianBlur*)effects[activeEffect];
			//		//float intensity = temp->GetIntensity();
			//		//
			...
			shader->SetUniform("u_SpecOnly", (int)spec);
			shader->SetUniform("u_AmbOnly", (int)amb);
			bloomEffect->SetBloomUniform("useBloom", (int)useBloom);
			shader->SetUniform("u_NoTex", (int)noTex);
		...
		Texture2D::sptr Banana = Texture2D::LoadFromFile("images/Banana_Plants_BaseColor.png");
		Texture2D::sptr Coco = Texture2D::LoadFromFile("images/CocoColor.png");
		Texture2D::sptr Brush = Texture2D::LoadFromFile("images/brushColor.png");

		Texture2D::sptr Sand = Texture2D::LoadFromFile("images/Desert.jpg");
		...
		desc.Width = 1;
		desc.Height = 1;
		desc.Format = InternalFormat::RGBA16;
		...
		ShaderMaterial::sptr sand = ShaderMaterial::Create();
		sand->Shader = shader;
		sand->Set("s_Diffuse", Sand);
		sand->Set("s_Specular", noSpec);
		sand->Set("u_Shininess", 2.0f);
		sand->Set("u_TextureMix", 0.0f);
		...
		//BP
		ShaderMaterial::sptr BP = ShaderMaterial::Create();
		BP->Shader = shader;
		BP->Set("s_Diffuse", Banana);
		BP->Set("s_Specular", noSpec);
		BP->Set("u_Shininess", 8.0f);
		BP->Set("u_TextureMix", 0.0f);

		//Coco
		ShaderMaterial::sptr CT = ShaderMaterial::Create();
		CT->Shader = shader;
		CT->Set("s_Diffuse", Coco);
		CT->Set("s_Specular", noSpec);
		CT->Set("u_Shininess", 8.0f);
		CT->Set("u_TextureMix", 0.0f);

		//Brush
		ShaderMaterial::sptr Shrub = ShaderMaterial::Create();
		Shrub->Shader = shader;
		Shrub->Set("s_Diffuse", Brush);
		Shrub->Set("s_Specular", noSpec);
		Shrub->Set("u_Shininess", 8.0f);
		Shrub->Set("u_TextureMix", 0.0f);

		GameObject obj1 = scene->CreateEntity("Ground"); 
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/plane.obj");
			obj1.emplace<RendererComponent>().SetMesh(vao).SetMaterial(sand);
		}
		...
		EnvironmentGenerator::AddObjectToGeneration("models/BP.obj", BP, 100,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/Coco.obj", CT, 50,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/brush.obj", Shrub, 40,
		...
		GameObject gaussianBlurObject = scene->CreateEntity("Blur Effect");
		{
			blur = &gaussianBlurObject.emplace<GaussianBlur>();
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			//Second blur pass
			//effects[activeEffect]->ApplyEffect2(basicEffect);
